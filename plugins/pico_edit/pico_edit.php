...
final class Pico_Edit extends AbstractPicoPlugin
{
    ...
    {
        // Default options
        if (!isset($config['pico_edit_404'])) {
            $config['pico_edit_404'] = true;
        }
        if (!isset($config['pico_edit_options'])) {
            $config['pico_edit_options'] = true;
        }
        // Parse extra options
    ...
    public function onRequestUrl(&$url)
    {
        ...
        if ($url == $this->url.'/commit') {
            $this->do_commit();
        }
        if ($url == $this->url.'/git') {
            $this->do_git();
        }
        if ($url == $this->url.'/pushpull') {
            $this->do_pushpull();
        }
        if ($url == $this->url.'/clearcache') {
            $this->do_clearcache();
        }
    }
    ...
    private function do_new()
    {
        ...
        if (empty($title)) {
            die(json_encode(array( 'error' => 'Error: Invalid title' )));
        }
        ...
        if ($pos > 0) {
            ...
            if (empty($dir)) {
                die(json_encode(array( 'error' => 'Error: Invalid folder' )));
            }
        }

        $file = $this->slugify($name);
        if (empty($file)) {
            die(json_encode(array( 'error' => 'Error: Invalid page name' )));
        }
        ...
        if (!empty($dir)) {
            ...
            if (!is_dir($path)) {
                if (!mkdir($path)) {
                    die(json_encode(array( 'error' => 'Can\'t create folder' )));
                }
            }
        }
        ...
        if (file_exists($path)) {
            ...
            if (strlen($content) !== file_put_contents($path, $content)) {
    ...
    private function do_open()
    {
        ...
        } elseif ($this->getConfig('pico_edit_options')) {
            $conf = $this->getConfigDir() . '/options.conf';
            if (file_exists($conf)) {
                die(file_get_contents($conf));
            } else {
                die('Error: Invalid options file');
            }
        }
    }
